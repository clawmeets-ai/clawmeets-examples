...
1. **Final-round prep packet** — *"Build my final-round prep packet for **[role]** at **[company]** from my attached resume and the JD. All deliverables go to `deliverables/`. `@career_coach`: tailor the resume bullets to the JD; write `deliverables/resume-tailored.md`. `@researcher`: WebSearch recent launches, leadership posts, and public engineering writing — public sources only (skip levels.fyi / Blind / Glassdoor and flag anything you can't ground). Write `deliverables/company-brief.md` with a one-paragraph what-they-do, 5 recent moves with source URLs, 3 talking points, and 3 risks I should ask about. `@interview_coach`: read the resume + brief and write `deliverables/star-bank.md` (6 STAR answers covering scope / ownership / conflict / failure) plus `deliverables/likely-questions.md` (10 questions tuned to this company / role). `@outreach_writer`: write `deliverables/thank-you-template.md` with `[PLACEHOLDER]` slots. Coordinator: stitch `deliverables/PREP.md` — a one-page index plus the ONE thing I should rehearse hardest before the loop."*
