...
> Build my **go/no-go pack** for opening a **second [coffee shop] in [East Austin]** in the next 6 months. Budget: **$[350K] all-in**. Two candidate sites: **[1450 E 6th — 1,400 sqft, $6.5K/mo NNN]** and **[2100 Manor Rd — 1,100 sqft, $5.2K/mo NNN]**. All deliverables go to `deliverables/site-eval/`. `@market_analyst`: per-site memos (`site-A.md` / `site-B.md`) — trade area, competitors, scorecard, single-biggest-risk, go / go-with-changes / no-go. `@finance`: 24-month P&L driven by traffic × conversion × ticket + opening capex with 10–20% contingency → `p-and-l.md`. `@marketing`: positioning + pre-opening + grand opening + post-launch weekly themes + local press / influencer list + loyalty structure → `launch-90day.md`. Coordinator: stitch `INDEX.md` — recommended site, the single most important reason, the single biggest risk, the ONE decision to make this week.
